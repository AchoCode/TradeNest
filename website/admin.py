...
def admin_dashboard():
    ...
    #         withdraw_usr = User.query.get(withdrawal.user_id)
    #         Amount = float(withdrawal.Amount)
    #         details = {
    #             'id' : withdrawal.id,
    #             'Username': f'{withdraw_usr.First_name} {withdraw_usr.Last_name}',
    #             'User_balance': withdraw_usr.Balance,
    #             'User_avail_balance': withdraw_usr.Available_balance,
    #             'Wallet': withdrawal.Wallet,
    #             'Fee_charge' : withdrawal.Fee,
    #             'Amount': Amount,
...
@admin.route('/get-withdrawwal', methods=['GET','POST'])
def get_withdrawal():
    #load json data
    data = json.loads(request.data)
    ...
    withdrawals = Transactions.query.filter_by(Title='Withdrawal', Status=PENDING).all()
    if transaction in withdrawals:
        #parse a dictionary with transaction info
            withdraw_usr = User.query.get(transaction.user_id)
            Amount = float(transaction.Amount)
            details = {
                'id' : transaction.id,
                'Username': f'{withdraw_usr.First_name} {withdraw_usr.Last_name}',
                'Email' : withdraw_usr.Email,
                'User_avail_balance': withdraw_usr.Available_balance,
